...
public class SWTGate extends AbstractSWTVertex {
	...
	protected void paintInternal(GC gc, Device device) {
		...
		/*TODO male hier ein Rectangle mit dem zugehörigen Color Scheme des Checkpointes dazu
		 * 
		 */
		//@author bel
		//Only an output gate have an checkpoint
		if (!isInputGate()) {
			
			ManagementVertex managementVertex = this.mangementGate.getVertex();
			String checkpointState = managementVertex.getCheckpointState();
			
			//get the color
			Color checkpointColor = null;
			if (checkpointState == null || checkpointState.equals("UNDECIDED")) {
				checkpointColor = ColorScheme.getCheckpointUndecidedColor(device);
			} else if (checkpointState.equals("PARTIAL")) {
				checkpointColor = ColorScheme.getCheckpointPartialColor(device);
			} else if (checkpointState.equals("COMPLETE")) {
				checkpointColor = ColorScheme.getCheckpointCompleteColor(device);
			} else if (checkpointState.equals("NONE")) {
				checkpointColor = ColorScheme.getCheckpointNoneColor(device);
			}
			
			gc.setBackground(checkpointColor);
			gc.fillOval(getEdgeSourceX() - getWidth()/8, getEdgeSourceY() - getHeight()/2, getWidth()/4, getHeight()*2);

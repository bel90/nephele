...
public class LongTest {
	 ...
	 */
	private static final int RECORDS_TO_GENERATE = 32 * 512; //256 * 512
	...
	public static void stopNephele() {
		//Wait, so the Visualization can take all needed Events from the JobManager
		
		try {
			System.out.println("Wait for the visualization.");
			Thread.sleep(5000);
	...
	public final static class InnerTask extends AbstractTask {
		...
		public void registerInputOutput() {
			this.recordWriter = new RecordWriter<JobRecord>(this);
			this.recordReader = new MutableRecordReader<JobRecord>(this);
		}

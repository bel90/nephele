...
public class RuntimeOutputGate<T extends Record> extends AbstractGate<T> implements OutputGate<T> {
	 ...
	 */
	public void addOutputChannel(AbstractOutputChannel<? extends Record> channel) {
		if (this.getOutputChannel(channel.getID()) == null) {
			this.outputChannels.add((AbstractOutputChannel<T>) channel);
		}
	}
	public AbstractOutputChannel<T> getOutputChannel(ChannelID outputChannelID) {
		...
		LOG.debug("Cannot find output channel with ID " + outputChannelID );
		return null;
	}
	/**
	...
	public  ArrayList<ChannelID> getOutputChannelIDs() {
		ArrayList<ChannelID> channelIDs = new ArrayList<ChannelID>();
		Iterator<AbstractOutputChannel<T>> iter = this.outputChannels.iterator();
		while(iter.hasNext()){
			channelIDs.add(iter.next().getID());
		}
		return channelIDs;
	}
	/**

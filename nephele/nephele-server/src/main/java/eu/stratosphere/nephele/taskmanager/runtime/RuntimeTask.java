...
public final class RuntimeTask implements Task, ExecutionObserver {
	...
	private EphemeralCheckpoint ephemeralCheckpoint = null;
	...
	public RuntimeTask(final ExecutionVertexID vertexID, final RuntimeEnvironment environment,
		...
		this.environment.setRuntimeTask(this);
	}
...
//		}
	...
	public void startExecution() {
		...
		//When the execution is startet: 
		//send an event with the initial Checkpoint State
		switch(this.initialCheckpointState) {
		case UNDECIDED:
			try {
				checkpointDecisionReasonIsSet(CheckpointDecisionReason.INITIAL_UNDECIDED);
			} catch (Exception e) {
			}
			break;
		...
		case NONE:
			try {
				checkpointDecisionReasonIsSet(CheckpointDecisionReason.INITIAL_NONE);
			} catch (Exception e) {
			}
			break;
		...
		case PARTIAL:
			try {
				checkpointDecisionReasonIsSet(CheckpointDecisionReason.INITIAL_PARTIAL);
			} catch (Exception e) {
			}
			break;
		...
		default:
			try {
				checkpointDecisionReasonIsSet(CheckpointDecisionReason.UNDECIDED);
			} catch (Exception e) {
			}
			break;
		}
	...
	/**
	 * Propagate the CheckpointDecisionReason Event to the Job Manager
	 * @author bel
	 */
	public void checkpointDecisionReasonIsSet(final CheckpointDecisionReason reason) throws IOException,
			InterruptedException {
		
		this.taskManager.checkpointDecisionReasonIsSet(this.environment.getJobID(), this.vertexID, reason);
	}
	
	public void recordSkipped() {
		//TODO delegate this to make an RecordSkipped(true) Event
		try {
			this.taskManager.recordSkipped(this.environment.getJobID(), this.vertexID);
		} catch (Exception e) {
		} 
	}
	
	/**
	 * Propagate the CheckpointSize Event to the Job Manager
	 * @author bel
	 */
	public void newCheckpointSize(int checkpointSize) {
		try {
			this.taskManager.newCheckpointSize(this.environment.getJobID(), this.vertexID, checkpointSize);
		} catch (Exception e) {
		} 
	}
	 ...
	 * @author bel
	 */
	public void changeCheckpointState(CheckpointState newCheckpointState) {
		if (this.ephemeralCheckpoint != null) {
			try {
				this.ephemeralCheckpoint.changeCheckpointDecisionSynchronously(newCheckpointState);
				//this.ephemeralCheckpoint.changeCheckpointDecisionSynchronously(false);
			} catch (Exception e) {
			} 
		}
	}
	...
	/**
	 * Here the EphemeralCheckpoint can register itself to the RuntimeTask
	 * @param ec
	 */
	public void setEphemeralCheckpoint(EphemeralCheckpoint ec) {
		this.ephemeralCheckpoint = ec;
		//System.out.println("EphemeralCheckpoint sent themselve to RuntimeTask" + this.vertexID);
	}

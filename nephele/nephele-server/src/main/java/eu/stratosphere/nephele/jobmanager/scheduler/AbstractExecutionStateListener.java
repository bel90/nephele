		...
		if (newExecutionState == ExecutionState.CANCELED || newExecutionState == ExecutionState.FINISHED) {
			...
			synchronized (eg) {
				...
				if (this.scheduler.getVerticesToBeRestarted().remove(this.executionVertex.getID()) != null) {
					...
					this.executionVertex.updateExecutionState(ExecutionState.ASSIGNED, "Restarting " + this.executionVertex.getName() +" as part of recovery");

					// Run through the deployment procedure
					this.scheduler.deployAssignedVertices(this.executionVertex);
					ExecutionVertex dupVertex = this.executionVertex.duplicateVertex(false);
					this.scheduler.deployAssignedVertices(dupVertex);
					return;
				}
			}
		}
...
//			try {
//				report = this.executionVertex.getFailureReport();
//				System.out.println("Got Failure Report: ");
			...
			if (this.executionVertex.decrementRetriesLeftAndCheck()) {
				...
				try {
					...
					recover = RecoveryLogic.recover(this.executionVertex, this.scheduler.getVerticesToBeRestarted(),
						assignedVertices, this.scheduler);
				...
				if (recover) {
					...
					if (RecoveryLogic.hasInstanceAssigned(this.executionVertex)) {
						// Reconstruct deployment in case we changed the configuration
						this.executionVertex.constructDeploymentDescriptor();
						// Run through the deployment procedure
						...
						final Iterator<ExecutionVertex> it = this.scheduler.getVerticesToBeRestarted().values()
							...
							while (it.hasNext()) {
								ExecutionVertex restartVertex = it.next();
								//System.err.println("Construct deployment for " + restartVertex.getNameWithIndex() + " after "  + this.executionVertex.getNameWithIndex()+ " failed");
								restartVertex.constructDeploymentDescriptor();
							}
					}
				...
				System.err.println("Vertex " + executionVertex.getName() + " does not have retries left.");
			}
		}
	}

...
public class EphemeralCheckpoint implements CheckpointDecisionRequester {
	...
	/**
	 * Size of the Checkpoint and number of envelopes
	 */
	private int checkpointSize = 0;
	private int numberOfEnvelopes = 0;
	
	/**
	 * Constants and tmp for calculating the criteria to send an event
	 */
	private static int SIZE = 65536 * 4; // 256 kb
	private static int ENVELOPES = 20;
	private int tmpSize = 0;
	private int tmpEnvelopes = 0;
	...
	public EphemeralCheckpoint(final RuntimeTask task, final int totalNumberOfOutputChannels, ArrayList<ChannelID> channelIDs, final boolean ephemeral) {
		...
		if (this.checkpointingDecision == CheckpointingDecisionState.CHECKPOINTING) {
			...
			for(int i = 0; i < this.channelIDs.size(); i++){	
				...
				writeThread.setEphemeralCheckpoint(this);
				writeThread.start();
				this.writeThreads.put(this.channelIDs.get(i),writeThread);
			}
		}
		
		this.task.setEphemeralCheckpoint(this);
	}
	...
	/**
	 * 
	 * @param newCheckpointingDecisionState
	 * @author bel
	 * @throws InterruptedException 
	 * @throws IOException 
	 */
	public void changeCheckpointDecisionSynchronously(CheckpointState newCheckpointState) 
			throws IOException, InterruptedException {
		/**TODO
		 * Wird in dieser Funktion this.task.checkpointStateChanged(CheckpointState check); verwendet
		 * kommt es zu einem kryo Fehler. Warum? Dieser Aufruf wird an mehreren Stellen ohne Probleme gemacht
		 * und CheckpointState ist auch in ServerTypeUtils registriert.
		 */
		LOG.info("Try Changing the Checkpoint to " + newCheckpointState);
		//System.out.println("EphemeralCheckpoint changeCheckpointDecisionSynchronously");
		
		switch (this.checkpointingDecision) {
		case UNDECIDED:
			//System.out.println("undecided");
			switch (newCheckpointState) {
			...
			case PARTIAL:
				...
				write();
				//this.task.checkpointStateChanged(CheckpointState.PARTIAL);
				this.task.checkpointDecisionReasonIsSet(CheckpointDecisionReason.USER_CHECKPOINTING);
				break;
			...
			case NONE:
				...
				destroy();
				//this.task.checkpointStateChanged(CheckpointState.NONE);
				this.task.checkpointDecisionReasonIsSet(CheckpointDecisionReason.USER_NO_CHECKPOINTING);
				break;
		...
		case CHECKPOINTING:
			...
			case NONE:
				...
				CheckpointUtils.removeCheckpoint(this.task.getVertexID());
		...
		case NO_CHECKPOINTING:
			//System.out.println("no_checkpointing");
			break;
		}
		//System.out.println("EphemeralCheckpoint changeCheckpointDecisionSynchronously2");
	...
	public void forward(final TransferEnvelope transferEnvelope) throws IOException, InterruptedException {
		...
		if (this.checkpointingDecision == CheckpointingDecisionState.UNDECIDED) {
			...
			if (this.asyncronousCheckpointDecisionRequested) {
				...
				setCheckpointDecisionSynchronously(getDecision());
			}
	 ...
	 * @author bel
	 */
	private boolean getDecision() throws IOException, InterruptedException {
		CheckpointDecisionReason reason = CheckpointDecision.getDecisionReason(task);
		
		//send the checkpoint decision information to the task
		this.task.checkpointDecisionReasonIsSet(reason);
		...
		case NETWORKNOT: return false; 

		case CPU: return true;
		
		case INOUTRATE: return true;
		
		case DYNAMICNOT: return false;
	...
	public void addCheckpointSize(int addSize) {
		
		this.checkpointSize += addSize;
		this.numberOfEnvelopes++;
		
		if ((this.checkpointSize - this.tmpSize > SIZE) || 
				(this.numberOfEnvelopes - this.tmpEnvelopes > ENVELOPES)) {
			//TODO schicke Event los
			
			this.task.newCheckpointSize(this.checkpointSize);
			//System.out.println(this.checkpointSize);
			
			this.tmpSize = this.checkpointSize;
			this.tmpEnvelopes = this.numberOfEnvelopes;
		} 

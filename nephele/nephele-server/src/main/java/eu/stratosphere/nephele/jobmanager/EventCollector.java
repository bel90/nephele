...
import eu.stratosphere.nephele.executiongraph.CheckpointDecisionReasonListener;
import eu.stratosphere.nephele.executiongraph.CheckpointSizeListener;
...
import eu.stratosphere.nephele.executiongraph.RecordSkippedListener;
...
public final class EventCollector extends TimerTask implements ProfilingListener, CheckpointStateListener,
			CheckpointDecisionReasonListener, CheckpointSizeListener, RecordSkippedListener {
		...
		synchronized (this.collectedEvents) {
			...
			if (eventsForJob != null) {
				...
				while (it.hasNext()) {
					...
					//if (!isManagementEvent) { //läuft fehlerfrei
						//CheckpointStateChangeEvent hatte in ManagementTypeUtils gefehlt!
					//if (!isManagementEvent || (includeManagementEvents && !(event instanceof CheckpointStateChangeEvent))) {
						eventList.add(event);
					}
				}
			}
	...
	public void registerJob(final ExecutionGraph executionGraph, final boolean profilingAvailable,
		...
		while (it.hasNext()) {
			...
			vertex.registerCheckpointDecisionReasonListener(this);
			
			vertex.registerCheckpointSizeListener(this);
			
			vertex.registerRecordSkippedListener(this);
		}
	 ...
	 */
	@Override
	public void checkpointDecisionReason(JobID jobID, ExecutionVertexID vertexID, CheckpointDecisionReason reason) {
		
		addEvent(jobID, new CheckpointDecisionReasonEvent(System.currentTimeMillis(), vertexID.toManagementVertexID(),
				reason.toString()));
	 ...
	 */
	@Override
	public void checkpointSize(JobID jobID, ExecutionVertexID vertexID, int size) {
		addEvent(jobID, new CheckpointSizeEvent(System.currentTimeMillis(), vertexID.toManagementVertexID(),
				size));
	}
	 ...
	 */
	@Override
	public void recordSkipped(JobID jobID, ExecutionVertexID vertexID) {
		addEvent(jobID, new RecordSkippedEvent(System.currentTimeMillis(), vertexID.toManagementVertexID()));
	}
}

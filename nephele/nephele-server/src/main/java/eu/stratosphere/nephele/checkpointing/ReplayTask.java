...
public final class ReplayTask implements Task {
	...
	public ReplayTask(final RuntimeTask encapsulatedTask, final TaskManager taskManager, Environment re) {
		
		this.vertexID = encapsulatedTask.getVertexID();
		if(re == null){
			re = encapsulatedTask.getEnvironment();
		}else{
			RuntimeEnvironment curEnv = (RuntimeEnvironment) encapsulatedTask.getEnvironment();
			for ( int i = 0 ; i < re.getNumberOfOutputGates(); i++){
				RuntimeOutputGate<? extends Record> gate = curEnv.getOutputGate(i);
				RuntimeOutputGate<? extends Record> nGate = (RuntimeOutputGate<? extends Record>) re.getOutputGate( gate.getGateID());
				if(gate.getNumberOfOutputChannels() != nGate.getNumberOfOutputChannels()){
					for(int j = 0; j < nGate.getNumberOfOutputChannels(); j++){
					 AbstractOutputChannel<? extends Record> channel = nGate.getOutputChannel(j);
						gate.addOutputChannel((AbstractOutputChannel<? extends Record>) channel);
					}
				}
		...
		this.environment = new CheckpointEnvironment(this.vertexID, re,
			CheckpointUtils.hasLocalCheckpointAvailable(this.vertexID),
			CheckpointUtils.hasCompleteCheckpointAvailable(this.vertexID),
			this.outputBrokerMap);
	...
	public void markAsFailed() {
		...
		//LOG.error("Execution thread of ReplayTask died unexpectedly");
		
		this.replayTaskExecutionState = ExecutionState.FAILED;
		this.failureRep = this.environment.getFailureReport();
		
		reportExecutionStateChange(true, "Execution thread of ReplayTask died unexpectedly");
	}
	...
	private void cancelOrKillExecution(final boolean cancel) {
		...
		if (cancel) {
			...
			if (this.encapsulatedTask != null) {
				this.encapsulatedTask.cancelExecution();
				try {
...
//			if(!encapsulatedThread.isAlive() && !replayThread.isAlive()){
//				System.err.println("All dead already");
//			}
	...
	private static ExecutionState determineOverallExecutionState(final ExecutionState unchangedExecutionState,
		...
		if (changedExecutionState == ExecutionState.CANCELED && unchangedExecutionState == ExecutionState.CANCELING) {
			return ExecutionState.CANCELED;
		}
	...
	public RuntimeTask getEncapsulatedTask() {

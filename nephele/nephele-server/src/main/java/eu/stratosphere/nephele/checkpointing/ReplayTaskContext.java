...
final class ReplayTaskContext implements TaskContext, BufferProvider, AsynchronousEventListener {
	...
	ReplayTaskContext(final ReplayTask task, final RoutingService routingService,
		...
		if (previousBufferPoolOwner instanceof ReplayTaskContext) {
			this.previousBufferPoolOwner = ((ReplayTaskContext) previousBufferPoolOwner).getPreviousBufferPoolOwner();	
		} else{
		this.previousBufferPoolOwner = previousBufferPoolOwner;
		}
		if (previousBufferPoolOwner == null ||previousBufferPoolOwner instanceof ReplayTaskContext  ) {
			this.localBufferPool = new LocalBufferPool(1, false, this);
		} else {
			
			if (!(this.previousBufferPoolOwner instanceof RuntimeTaskContext)) {
			...
			final RuntimeTaskContext rtc = (RuntimeTaskContext) this.previousBufferPoolOwner;
	...
	public LocalBufferPoolOwner getPreviousBufferPoolOwner() {
		return previousBufferPoolOwner;
	}
}

...
public final class DefaultRoutingService implements RoutingService, BufferProviderBroker {
	...
	public void unregister(final ExecutionVertexID vertexID, final Task task) {
		...
		while (channelIterator.hasNext()) {
			...
			final ChannelID inputChannelID = channelIterator.next();
			final ChannelContext context = this.registeredChannels.remove(inputChannelID);
			//System.out.println("Unregistering Input Channel " + inputChannelID);
			if (context != null) {
				context.destroy();
			}
			this.receiverCache.remove(inputChannelID);
		}
	...
	public void updateRegistration(Task runningTask) {
		TaskContext taskContext = (TaskContext) this.localBufferPoolOwner.get(runningTask.getVertexID());
		Environment environment = runningTask.getEnvironment();
		final Set<GateID> outputGateIDs = environment.getOutputGateIDs();
		for (final Iterator<GateID> gateIt = outputGateIDs.iterator(); gateIt.hasNext();) {
			...
			for (final Iterator<ChannelID> channelIt = outputChannelIDs.iterator(); channelIt.hasNext();) {
				...
				if(previousContext == null){
					i++;
					OutputChannelContext outContext = outputGateContext.createOutputChannelContext(channelID, previousContext, true, this.mergeSpilledBuffers);
					if(taskContext instanceof RuntimeTaskContext){
						//Update number of output channels on ephemeral Checkpoint and Checkpoint writting Thread if CP exists
						EphemeralCheckpoint cp = ((RuntimeTaskContext) taskContext).getEphemeralCheckpoint();
						if(cp != null){
							cp.setNumberOfOutputChannels(outputChannelIDs.size());
						}
					}
					this.registeredChannels.put(channelID, outContext);
					if(outContext.getType() == ChannelType.INMEMORY){
						addReceiverListHint(outContext);
					}
					
				}
			}
		//	System.out.println(" Created "  + i + " new Channelcontexts " + environment.getTaskName() );
		}
		...
		for (final Iterator<GateID> gateIt = inputGateIDs.iterator(); gateIt.hasNext();) {
			...
			for (final Iterator<ChannelID> channelIt = inputChannelIDs.iterator(); channelIt.hasNext();) {
				...
				if(previousContext == null){
					i++;
					InputChannelContext inContext = inputGateContext.createInputChannelContext(channelID, previousContext);
					this.registeredChannels.put(channelID, inContext);
					if(inContext.getType() == ChannelType.INMEMORY){
						addReceiverListHint(inContext);
					}
				}
			}
	//		System.out.println(" Created "  + i + " new Channelcontexts " + environment.getTaskName() );
